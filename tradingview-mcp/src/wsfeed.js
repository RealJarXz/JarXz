...
export function registerWsFeedTools(server) {
  server.tool(
    'ws_quote',
    'Get a real-time price quote from TradingView servers via WebSocket. Works for crypto (BINANCE:BTCUSDT), stocks, forex, indices.',
    {
  ...
  server.tool(
    'ws_ohlcv',
    'Fetch historical OHLCV candles from TradingView via WebSocket. Supports any symbol and timeframe.',
    {
  ...
  server.tool(
    'ws_watchlist_quotes',
    'Fetch live quotes for all symbols in the rules.json watchlist via TradingView WebSocket.',
    {},
    async () => {
      try {
